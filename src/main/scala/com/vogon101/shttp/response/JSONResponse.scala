...
/**
  ...
  */
class JSONResponse(response: CloseableHttpResponse, preReadText:Option[String] = None) extends HTTPResponse(response, preReadText) {

  /**
    * Error value to give when parsing the stream failed
    */
  override val errorValue = """{"error": "Error when parsing stream or JSON"}"""
